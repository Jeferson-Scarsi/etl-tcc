...
def dim_status_pedido():
    ...
    # 4 - Seleciona as colunas necessárias para a tabela de dimensão do cliente
    df = df[['status_pedido', 'cd_origem_sistema', 'cd_execucao']]

    # 5 - Ordena os dados por todas as colunas para garantir a consistência na ordenação de forma ascendente
    df = df.sort_values(by=['status_pedido', 'cd_origem_sistema', 'cd_execucao'], ascending=True)

    # 6 - Remove as linhas duplicadas com base na coluna status_pedido.
    df = df.drop_duplicates(subset=['status_pedido'], keep='first')

    # 7 - Mapeia as colunas e insere os dados na tabela de dimensão do cliente no banco de dados
    with conn_dw.connect() as connection_dw:
