...
def dim_pagamento():
    ...
    sql = md.load_sql('dim_pagamento.sql')
    ...
    df = df.sort_values(by=['tipo_pagamento'], ascending=True)

    # 3 - Remove as linhas duplicadas com base na coluna tipo_pagamento.
    df = df.drop_duplicates(subset=['tipo_pagamento'], keep='first')

    # 4 - Seleciona as colunas necessárias para a tabela de dimensão do cliente
    df = df[['tipo_pagamento']]

    # 5 - Insere os dados na tabela de dimensão do cliente no banco de dados
    with conn_dw.connect() as connection_dw:
        df.to_sql(schema='dw', name='dim_pagamento', con=connection_dw, if_exists='append', index=False)
